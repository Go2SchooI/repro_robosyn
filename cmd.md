...
# 观察：L_state 能否快速下降、L_dyn 是否明显下降、当前步 vs 下一步谁更难、哪一维最难学。
# 用法一：一键过拟合测试（max_samples=1024, epochs=500, 每 50 epoch 及结束时打 per-dim MSE）
python -m belief_s1.train_s1 --data_dir data/s1_baoding --overfit_test
# 用法二：单域 + 指定样本数
python -m belief_s1.train_s1 --data_dir data/s1_baoding --domain mujoco --overfit_test --max_samples 512
